package com.sundogsoftware.spark.SparkScala
...
// "_" can be anything in the numberList. It's a shortchut for the verbose-ish syntax
...
// Like dictionaries in other languages (Python)
...
val exList = (1 to numberOfItems)
for (item <- exList) {if (item % 3 == 0) println(item)}
for (item <- exList.filter((x: Int) => x % 3 == 0)) {println(item)}
